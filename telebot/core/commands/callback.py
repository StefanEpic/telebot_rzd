...
from core.commands.utils.send_file import get_document, NAME_TO_ANSWER, NUMBER_TO_NAME
from core.keyboards import safety_and_health_keyboard, list_keyboard


async def safety_and_health(call: CallbackQuery):
    await call.message.answer(
        text='🤖 Охрана труда:',
        reply_markup=safety_and_health_keyboard()
    )


async def get_list(call: CallbackQuery, callback_data: Menu):
    ...
    if callback_data.need_pagination:
        await call.message.edit_reply_markup(
            reply_markup=list_keyboard(data, callback_data.title, int(page), ceil(int(data['count']) / 5))
        )
    else:
        await call.message.answer(
...
async def download_file(call: CallbackQuery, callback_data: DownloadFile):
