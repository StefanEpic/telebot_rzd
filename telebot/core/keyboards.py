...
def menu_keyboard() -> InlineKeyboardMarkup:
    ...
    keyboard_builder.button(text="🗂 Охрана труда", callback_data="occupational_safety_and_health")
    keyboard_builder.button(
        text="📁 Бланки заявлений",
        callback_data=Menu(title="07", page=1, need_pagination=False),
    )
    keyboard_builder.button(
        text="📁 Техническая учеба",
...
def safety_and_health_keyboard() -> InlineKeyboardMarkup:
    keyboard_builder = InlineKeyboardBuilder()
    keyboard_builder.button(
        text="📁 Инструкции",
...
def list_keyboard(data: dict, dir_name: str, current_page: int, page_count: int) -> InlineKeyboardMarkup:
    keyboard_builder = InlineKeyboardBuilder()
    row = []
    ...
    if page_count > 1:
        if current_page != 1:
            ...
            keyboard_builder.button(text="⛔️", callback_data="stop")
